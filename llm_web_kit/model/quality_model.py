...
from llm_web_kit.model.basic_functions.features import (
    BULLET_POINT_SYMBOLS, content2lines, content2words, get_content_len,
...
class QualityModel:
    ...
    def predict_with_content(self, content: str):
        # 停用词相关
...
def get_quality_model(language, content_style):
    model_name = _model_resource_map[f'{language}-{content_style}']
...
def quality_filter(data_dict: Dict[str, Any], language: str, content_style: str):
    ...
    # content_list to content?
    return model.predict_with_content(data_dict['content'])
