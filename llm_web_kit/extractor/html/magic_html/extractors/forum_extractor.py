...
class ForumExtractor(BaseExtractor):
    ...
    def extract(self, html='', base_url='') -> dict:
        ...
        # 获取 main_ids 并转为集合
        main_ids = set(body_tree.xpath(f'.//@Unique_ID'))
        if not main_ids:
            main_ids = {-1}

        # 移除 normal_tree 中已处理的节点
        ...
        for c_xpath in Forum_XPATH:
            nodes_to_process = normal_tree.xpath(c_xpath)
            for x in nodes_to_process:
                if "'post-'" in c_xpath:
                    if not (re.findall(r'post-\d+', x.attrib.get('id', '').lower()) or
                            re.findall(r'post_\d+', x.attrib.get('id', '').lower())):
                        continue
                ...
                try:
                    ...
                    if node_id > max_main_id:
                        ...
                        for tmp_x in descendants:
                            tmp_id = int(tmp_x.attrib.get(Unique_ID, '0'))
                            if tmp_id > max_main_id:
                                suffix_div.append(tmp_x)
                                need_suffix = True
                            elif tmp_id < max_main_id:
        ...
        # 移除 unique_id 属性
        for elem in body_tree.iter():
            if Unique_ID in elem.attrib:
                del elem.attrib[Unique_ID]
