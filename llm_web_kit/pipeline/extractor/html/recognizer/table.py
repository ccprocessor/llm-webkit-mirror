...
class TableRecognizer(BaseHTMLElementRecognizer):
    ...
    def recognize(self, base_url:str, main_html_lst: List[Tuple[str,str]], raw_html:str) -> List[Tuple[str,str]]:
        ...
        for cc_html, o_html in main_html_lst:
            if self.is_contain_cc_html(cc_html):
                ...
                continue   
            for has_table, cc_table, o_table in self.contain_table_elements(cc_html):
    ...
    def contain_table_elements(self, raw_html: str) -> List[Tuple[bool, str, str]]:
        ...
        def process_direct_children(element):
            for child in element.children:
                if isinstance(child, Tag):
                    if child.name == 'table':
                        table_type = "simple"
                        # Check if the current table is complex by iterating only within this table
        ...
        # Start processing from the root of the document, but only process its direct children
...
if __name__ == '__main__':
    ...
    main_html_lst = [("<cccode>hello</cccode>", "<code>hello</code>"),
