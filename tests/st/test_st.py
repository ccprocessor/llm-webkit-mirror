        ...
        with open(self.sourcePath, 'r') as f:
            ...
            for fileName in files:
                ...
                try:
                    ...
                    with open(os.path.join(self.root, 'bench/data/', files[fileName]['groundtruth_filepath']), 'r') as f:
                        groundtruth = json.loads(f.readline().strip())
                    # 断言equation-interline, paragraph.equation-inline和list.equation-inline元素数一致
                    self.assertEqual(statics.get('equation-interline'), groundtruth.get('statics', {}).get('equation-interline'))
                    self.assertEqual(statics.get('paragraph.equation-inline'), groundtruth.get('statics', {}).get('paragraph.equation-inline'))
                    self.assertEqual(statics.get('list.equation-inline'), groundtruth.get('statics', {}).get('list.equation-inline'))
