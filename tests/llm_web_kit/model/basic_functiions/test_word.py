...
from llm_web_kit.model.basic_functions.word import (  # noqa: E402
    build_stop_word_set, filter_stop_word, get_stop_word_en_zh_set)
...
class TestWord(unittest.TestCase):
    ...
    def test_build_stop_word_set_en_only(self):
        # 模拟 nltk 库行为
        with patch('nltk.corpus.stopwords.words', return_value=['and', 'or', 'but']):
    ...
    def test_get_stop_word_en_zh_set(self):
        # 使用之前的测试来模拟
        with patch('llm_web_kit.model.basic_functions.word.build_stop_word_set') as mocked_build:
            mocked_build.return_value = {'的', 'is', 'the'}
            result = get_stop_word_en_zh_set()
            self.assertEqual(result, {'的', 'is', 'the'})
            mocked_build.assert_called_once_with(include_zh=True, include_en=True)
